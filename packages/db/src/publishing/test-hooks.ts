...
export type PublishingTestHooks = {
  ...
  afterDraftGalleryReplaced?: (input: {
    contentVersionId: string;
  }) => Promise<void>;
...
export async function runAfterDraftGalleryReplaced(input: {
  contentVersionId: string;
}): Promise<void> {
  const hook = hooks?.afterDraftGalleryReplaced;
  if (!hook) {
    return;
  }
...
export async function runBeforeAuditEventInserted(input: {
  contentItemId: string;
  versionId: string | null;
  eventType: string;
}): Promise<void> {
  const hook = hooks?.beforeAuditEventInserted;
  if (!hook) {
    return;
  }
