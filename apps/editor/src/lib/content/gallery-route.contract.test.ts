...
const galleryRoute = path.join(
  fileURLToPath(
    new URL("../../app/api/content/[contentItemId]/gallery/route.ts", import.meta.url),
  ),
);

describe("article gallery route contract", () => {
  it("uses the editor write perimeter and CONTENT_EDIT", () => {
    const source = readFileSync(galleryRoute, "utf8");
    ...
    assert.equal(source.includes("parseDraftGalleryBody"), true);
    assert.equal(source.includes("setDraftVersionGallery"), true);
