...
import { listContentReviewHistory } from "@magazine/db/editor";
...
import {
  parseContentItemId,
  parseReviewHistorySearchParams,
...
export async function GET(
  ...
  return withEditorRead(request, CAPABILITY.CONTENT_READ, async (session) => {
    ...
    const filters = parseReviewHistorySearchParams(new URL(request.url));
    const result = await listContentReviewHistory(
      id,
